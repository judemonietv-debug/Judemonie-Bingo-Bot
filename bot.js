...
const BOT_USERNAME_FIXED = 'BinGo_bsc_bot'; // <--- GUARANTEED BOT USERNAME FIX
...
function getKeyboard() {
  return bot.keyboard([
    ['/tasks', '/points'],
    ['/myrefs', '/referral', '/withdraw ' + MIN_WITHDRAWAL]
  ], { resize: true });
}
...
bot.on(['/myrefs'], (msg) => {
  ...
  if (referredUsers.length === 0) {
    ...
    referredUsers.slice(0, 10).forEach((u, i) => {
      // Show username if available, otherwise use a masked ID
      const displayUsername = u.username && u.username !== 'Anonymous' ? `@${u.username}` : `User ID: \`${u.userId.substring(0, 4)}...\``;
      text += `${i + 1}. ${displayUsername}\n`;
    });
...
bot.on(['/referral'], (msg) => {
  ...
  // FIX: Use the hardcoded bot username for the correct link
...
bot.on('text', (msg) => {
  ...
  if (captchaState[userId] && captchaState[userId].answer) {
    if (text === captchaState[userId].answer) {
      ...
      // Award referrer bonus (Referral ID is saved in startNewUser but awarded here after captcha is solved)
...
bot.on(['/start'], (msg) => {
  ...
  if (user && user.points === 0 && user.referrerId) {
      // User exists temporarily from referral, but hasn't solved captcha. Restart captcha flow.
      startNewUser(msg);
    ...
    // Truly new user - Start Captcha Flow
    startNewUser(msg);
  }
});
