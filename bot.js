...
// FIX: Using /tmp for all writable files on Render
const DATA_FOLDER = process.env.TMPDIR ? path.join(process.env.TMPDIR, 'data') : './data';

// --- File Paths ---
const USERS_FILE = path.join(DATA_FOLDER, 'users.json'); // FIX: Use path.join
...
function readJSON(file) {
  try {
    ...
    // Ensure that if the file is empty, we return the correct default structure
    if (!data.trim()) {
