...
function getKeyboard() {
  return bot.keyboard([
    ['/tasks', '/points'],
    ['/referral', '/withdraw ' + MIN_WITHDRAWAL] // Show min withdrawal amount
  ], { resize: true });
}
...
bot.on('callbackQuery', async (msg) => {
  ...
  if (data.startsWith('claim_')) {
    ...
    if (taskId === 'join_tg_channel') {
      ...
      bot.sendMessage(userId, `🎉 You completed the X follow task! You now have ${user.points + task.points} points. (We trust you!)`, { replyMode: 'Markdown', replyMarkup: getKeyboard() });
...
bot.on('text', (msg) => {
  ...
  if (captchaState[userId] && captchaState[userId].answer) {
    if (text === captchaState[userId].answer) {
      ...
      let user = getUser(userId);
      if (!user) { // Re-initialize user if they solved captcha after the initial /start fail
        user = { userId, username: msg.from.username || 'Anonymous', points: 100, wallet: null };
        saveUser(user);
      }
...
function startNewUser(msg) {
  ...
  // 2. Referral Check (Award points to referrer only if user is new AND solves captcha later)
  if (referrerId && referrerId !== userId) {
    let referrer = getUser(referrerId);
    if (referrer) {
      addPoints(referrerId, 25); // Referral bonus
      bot.sendMessage(referrerId, `🎁 You received a 25 point bonus because a user you referred started the bot!`);
    }
  }
}
