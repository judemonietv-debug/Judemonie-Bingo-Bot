...
// ----------------- Configuration -----------------
const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN; // READS TOKEN FROM RENDER ENVIRONMENT
...
const keyboard = bot.keyboard([
    ['/points', '/leaderboard'],
    ['/referral', '/withdraw 5000'], // Suggesting minimum withdrawal
    ['/menu'] 
...
bot.on(['/start', '/menu'], (msg) => {
  ...
  const options = { replyMarkup: keyboard }; // Attach the custom keyboard

  if (!user) {
    user = { userId, username: msg.from.username || 'Anonymous', points: 100 };
    saveUser(user);
    bot.sendMessage(userId, `🎉 Welcome to BinGo! Use the buttons below to navigate!`, options);
  } else {
    bot.sendMessage(userId, `Main Menu:\nYou have ${user.points} points. Level: ${getLevel(user)}\n\nUse the buttons below for quick actions!`, options);
  }
});
...
bot.on(['/points'], (msg) => {
  ...
  bot.sendMessage(msg.from.id, `You have ${user.points} points. Level: ${getLevel(user)}`, {replyMarkup: keyboard});
});
...
bot.on(['/leaderboard'], (msg) => {
  ...
  bot.sendMessage(msg.from.id, text, {replyMarkup: keyboard});
});
...
bot.on(['/withdraw'], (msg) => {
  ...
  if (parts.length !== 2) {
    return bot.sendMessage(msg.from.id, `Usage: /withdraw <amount>. Minimum withdrawal is ${MIN_WITHDRAWAL} points.`, {replyMarkup: keyboard});
  }
  // ... (rest of the /withdraw logic remains the same)
  const amount = parseInt(parts[1]);

  if (isNaN(amount) || amount <= 0) {
    return bot.sendMessage(msg.from.id, "Please enter a valid positive number for the amount.", {replyMarkup: keyboard});
  }
  if (amount < MIN_WITHDRAWAL) {
    return bot.sendMessage(msg.from.id, `Minimum withdrawal amount is ${MIN_WITHDRAWAL} points.`, {replyMarkup: keyboard});
  }
  if (user.points < amount) {
    return bot.sendMessage(msg.from.id, `You only have ${user.points} points. You need ${amount - user.points} more points.`, {replyMarkup: keyboard});
  }
